...
// profissional (Módulo 14) — só cartão, mesmo preço avulso/mensal em cada
// moeda (€9,90 / R$58,00), conforme decidido (ver Termos de Uso seção 4).
// metadata.audience = 'professional' é o que diferencia esta sessão no
// stripe-webhook, que precisa resolver professional_subscriptions em vez de
// subscriptions. country segue o mesmo padrão de create-checkout-session
// (cliente manda 'BR'|'INT' com base em isBrazil(), sem endereço de
// cobrança coletado).
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { returnUrl, plan, extraChildren, country } = await req.json();
    const baseUrl = returnUrl || Deno.env.get('APP_URL');
    const is30Days = plan === '30days';
    const isBRL = country === 'BR';
    const priceId = is30Days
      ? Deno.env.get(isBRL ? 'STRIPE_PRICE_ID_PROFESSIONAL_30DAYS_BRL' : 'STRIPE_PRICE_ID_PROFESSIONAL_30DAYS_EUR')
      : Deno.env.get(isBRL ? 'STRIPE_PRICE_ID_PROFESSIONAL_MONTHLY_BRL' : 'STRIPE_PRICE_ID_PROFESSIONAL_MONTHLY_EUR');
    ...
    if (extraCount > 0) {
      ...
      const extraPriceId = is30Days
        ? Deno.env.get(isBRL ? 'STRIPE_PRICE_ID_PROFESSIONAL_EXTRA_CHILD_30DAYS_BRL' : 'STRIPE_PRICE_ID_PROFESSIONAL_EXTRA_CHILD_30DAYS_EUR')
        : Deno.env.get(isBRL ? 'STRIPE_PRICE_ID_PROFESSIONAL_EXTRA_CHILD_MONTHLY_BRL' : 'STRIPE_PRICE_ID_PROFESSIONAL_EXTRA_CHILD_MONTHLY_EUR');
