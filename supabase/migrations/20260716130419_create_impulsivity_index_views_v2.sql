-- 1. Contagens brutas por criança (versão corrigida)
create or replace view public.v_impulsivity_raw_counts as
...
-- 2. Índice geral (0-100), fórmula exata da Viviane
create or replace view public.v_impulsivity_index as
select
  profile_id,
  cliques_antecipados,
...
-- 3. Classificação em texto, separada para não repetir a expressão gigante
create or replace view public.v_impulsivity_classification as
select
  profile_id,
  indice,
  case
...
-- 4. Três dimensões separadas + índice geral ponderado (0.40 / 0.35 / 0.25)
create or replace view public.v_impulsivity_dimensions as
select
  profile_id,
  cliques_antecipados,
  toques_repetitivos,
